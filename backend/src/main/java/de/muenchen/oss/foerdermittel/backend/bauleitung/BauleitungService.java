...
public class BauleitungService {
    ...
    private final de.muenchen.oss.foerdermittel.backend.bauleitung.BauleitungRepository bauleitungRepository;
    ...
    @PreAuthorize(Authorities.HAS_ANY_ROLE)
    public Page<Bauleitung> getAllBauleitungen(final int pageNumber, final int pageSize) {
        log.info("Get all Bauleitungen with Page {} and PageSize {}", pageNumber, pageSize);
        final Pageable pageRequest = PageRequest.of(pageNumber, pageSize);
        return bauleitungRepository.findAll(pageRequest);
    }

    @PreAuthorize(Authorities.HAS_ROLE_ADMIN)
    public Bauleitung createBauleitung(final Bauleitung bauleitung) {
        log.debug("Create Bauleitung {}", bauleitung);
        return bauleitungRepository.save(bauleitung);
    }
    ...
    public Bauleitung updateBauleitung(final Bauleitung bauleitung, final String bauleitungId) {
        ...
        return bauleitungRepository.save(foundBauleitung);
    }
